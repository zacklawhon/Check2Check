...
function TargetStep({ debts = [], strategy, onBack, onComplete }) {
    ...
    const sortedDebts = useMemo(() => {
        ...
        // For Avalanche and Hybrid, sort by highest interest rate first
        ...
        <div>
            ...
            <ul className="space-y-3">
                {sortedDebts.map((debt, index) => (
                    <li key={debt.id} className={`bg-gray-700 p-4 rounded-lg flex items-center justify-between ${index === 0 ? 'border-2 border-indigo-500' : ''}`}>
                        <div>
                            <div className="flex items-center gap-3">
                                <p className="font-bold text-lg text-white">{debt.label}</p>
                                {index === 0 && <span className="bg-indigo-500 text-white text-xs font-bold px-2 py-1 rounded-full">Recommended</span>}
                            </div>
