// filepath: c:\laragon\www\frontend\src\components\budget\modals\EditIncomeItemModal.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom'; 
import * as api from '../../../utils/api'

function EditIncomeItemModal({ isOpen, onClose, onSuccess, item, budgetId }) {
    ...
    const handleSave = async () => {
        if (!label || !amount || parseFloat(amount) <= 0) {
            setError('Please enter a valid label and amount.');
            return;
        }
        setLoading(true);
        setError('');
        try {
            ...
            const response = await api.updateIncomeInCycle(budgetId, payload);
            onSuccess(response);
        } catch (err) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50">
            <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md">
                <h2 className="text-2xl font-bold mb-4">Edit Income Item</h2>
                <p className="text-gray-400 mb-4">Editing "{item.label}" for this budget cycle only.</p>
...
export default EditIncomeItemModal;